...
;;;;;;;::;;;:;::;;;;::::;;:;::;;;;:::;;;::::;:;;:;::;;;:;::;:::;::;::;;::;;:;;:::;;:
;;;  		   	     
;;; baraci17@ku.edu.tr     Thu Oct 19 13:48:33 2017
;;;  		   	     
;;; Before you start:  		   	     
;;;  		   	     
...
(define (segment-length seg)
  (sqrt (+ (square (- (caar seg) (caadr seg))) (square (- (cadar seg) (cadadr seg)))))) 
...
(define (create-polygon ptlist)
  (append (create-list-recursive ptlist) (list (list (find-last-point ptlist) (car ptlist)))))

; iterative version
;(define (create-list-from-points lst points)
;  (if (null? (cdr points))
;      lst
;      (create-list-from-points (append lst (list (list (car points) (cadr points)))) (cdr points))))

; recursive version
(define (create-list-recursive points)
  (if (null? (cdr points))
      '()
      (cons (list (car points) (cadr points)) (create-list-recursive (cdr points)))))

; it returns the last point
(define (find-last-point lst)
  (if (null? (cdr lst))
      (car lst)
      (find-last-point (cdr lst))))
...
(define (add-all-points lst)
  (if (null? (cdr lst))
      (car lst)
      (add-all-points (cons (add-two-points (car lst) (cadr lst)) (cddr lst)))))
...
(define (centroid lst)
  (list (/ (car (add-all-points lst)) (length lst)) (/ (cadr (add-all-points lst)) (length lst))))
...
(define (get-x-points lst)
  (if (null? lst)
      '()
      (cons (car (car lst)) (get-x-points (cdr lst)))))

(define (get-y-points lst)
  (if (null? lst)
      '()
      (cons (cadr (car lst)) (get-x-points (cdr lst)))))

(define (centroid-alt lst)
  (list (/ (apply + (get-x-points lst)) (length lst)) (/ (apply + (get-y-points lst)) (length lst))))
...
; ANSWER: Because square takes a Number as an argument but in the definition above, the argument of the square is procedure.
...
; ANSWER: Abbreviating "Number -> Number" as "F"
; ANSWER: Binary Operation -> (F, F -> F)
...
; ANSWER: square-a-procedure expects F but instead of that, it gets deriv which has a type of F -> F (abbreviating "Number -> Number" as "F")
...
; ANSWER: ((thrice thrice) f) 0	= 3^3 = 27 calls to f. n should be 27. ((repeated f 27) 0)		   	     
...
(define (diagonal-line pt len)
  (lambda (t) (make-point (+ (x-of pt) (* t len (cos (atan 1))))
                          (+ (y-of pt) (* t len (sin (atan 1)))))))
...
;((diagonal-line '(4 4) (* 4 (sqrt 2))) 0.5)
; Value: (6.0 6.0)
...
(define (upside-down proc)
  (lambda (t) (make-point (x-of (proc t)) (- (y-of (proc 0)) (- (y-of (proc t)) (y-of (proc 0))))))) ; subtracting (y of line) from (y of initial point)
  		   	     
; test
;((upside-down (diagonal-line p1 (* 5 (sqrt 2)))) 1)
; Value: (5.0 -5.0)
;((upside-down (diagonal-line '(4 4) (* 6 (sqrt 2)))) 0.5)
; Value: (7.0 1.0)
...
; 1)		   	     
; Value: (10.0 10.0)
;((connect-ends (diagonal-line p1 (* 4 (sqrt 2)))
;               (diagonal-line p2 (* 5 (sqrt 2))))
; 0.6)
; Value: (5.0 5.0)
...
;((draw-connected-squeezed-to-window g1 200) unit-circle)
;((draw-connected-squeezed-to-window g2 200) alternative-unit-circle)
;((draw-points-squeezed-to-window g1 200) unit-circle)
;((draw-points-squeezed-to-window g2 200) alternative-unit-circle)

; ANSWER: There is a difference for the procedure draw-points. The gap among the points of alternative-unit-circle gets bigger as t increases
; since alternative-unit-circle uses t^2 unlike the unit circle.
; IMAGE FILEPATH: exercise14.png
...
(define (gosper-curve2 level initial-curve)  		   	     
...
(define (show-points-gosper window level number-of-points initial-curve)
  ((draw-points-on window number-of-points)
   ((squeeze-rectangular-portion -.5 1.5 -.5 1.5)
    (gosper-curve2 level initial-curve))))
...
(define (unit-circle-2 t)  		   	     
  (make-point (sin (* pi t))  		   	     
              (cos (* pi t))))

(define arc (put-in-standard-position unit-circle-2))
...
;((draw-connected-squeezed-to-window g1 200) (param-gosper 6 (lambda (n) (/ pi (+ n 2)))))
;((draw-connected-squeezed-to-window g2 200) (param-gosper 6 (lambda (n) (/ pi (expt 1.3 n)))))
; IMAGE FILEPATH: exercise18.png
...
(define (kochize level)
  (param-gosper (* 2 level) (lambda (x) (if (= (modulo x 2) 0) (/ -pi 6) (/ pi 6)))))
...
;((draw-connected-squeezed-to-window g1 200) (put-in-standard-position (kochize 2)))
;((draw-connected-squeezed-to-window g2 200) (put-in-standard-position (kochize 3)))
;((draw-connected-squeezed-to-window g3 200) (put-in-standard-position (kochize 4)))

; IMAGE FILEPATH: Koch_(2-3-4)_-30.png and Koch_(3-4-6)_-60.png
...
; I worked alone using only the course references materials.

(define (origami level)
  (param-gosper (* 2 level) (lambda (x) (cond ((= (modulo x 4) 0) (/ pi 4))
                                              ((= (modulo x 4) 1) (/ -pi 4))
                                              ((= (modulo x 4) 2) (/ pi 6))
                                              ((= (modulo x 4) 3) (/ pi 2))))))
...
(define (lotus-flower level)
  (param-gosper (* 2 level) (lambda (x) (cond ((= (modulo x 6) 0) (/ pi 6))
                                              ((= (modulo x 6) 1) (/ -pi 6))
                                              ((= (modulo x 6) 2) (/ pi 2))
                                              ((= (modulo x 6) 3) (/ pi 3))
                                              ((= (modulo x 6) 4) (/ pi 4))
...
((draw-connected-squeezed-to-window g1 200) ((translate 0.2 0.6) (put-in-standard-position (origami 3))))
((draw-connected-squeezed-to-window g2 200) ((translate 0.4 -0.6) (put-in-standard-position (slash 3))))
((draw-connected-squeezed-to-window g3 200) ((translate 0 0.8) (put-in-standard-position (lotus-flower 3))))

; IMAGE FILEPATH: exercise21.png
